    ...
    try {
      ...
      if (data.success) {
        ...
        setMessages(prev => [...prev, {
          ...
          isChartRequested: isChartRequested || !!(payload.chart_data || payload.visual),
          isTableRequested: isTableRequested || !!(payload.records && payload.records.length > 0),
          chartType: payload.chart_data?.chart_type || payload.visual?.type || chartType || "bar",
          chartLabel: payload.chart_data?.title || payload.visual?.title || q.slice(0, 40),
          visual: payload.chart_data || payload.visual || null,
          records: payload.records || data.records || null,
